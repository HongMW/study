...
<!-- 
	request 기본객체(HttpSetbletRequest)
	
	request기본객체는 jsp페이지에서 가장 많이 사용되는 객체로서 웹브라오저의 요청과 관련있는 객체이다.
	웹브라우저에서 웹사이트주소를 입력하면 웹브라우저는 해당 주서의 웹서버에 연결 한 후 요청정보를 전송
	하는데 이 요청 정보를 웹서버에 전달하는 객체가 request객체이다.
	
	A. request객체가 제공하는 기능
	
		a. 클라이언트(웹브라우저)와 관련된 정보
		b. 서버와 관련된 정보
		c. 클라이언트가 전송된 요청파라미터정보(http://localhost:8080?id=hong&pw=12345
		d. 클라이언트가 전송된 요청헤더정보(예 : http통신규약에 따른 정보)
		e. 클라이언트가 전송된 쿠키정보
		f. 속성처리정보
	...
	D. request객체의 요청헤더 관련 메서드
		...
		d. getIntHeader(String name)	: 지정한 헤터의 값을 정수값으로 리턴
