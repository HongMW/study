...
/*
 	...
 	1. 데이터타입분류
 		...
 		1) 기본타입(premetive type)
 			...
 			정수(nyte, short, char, int, long), 실수(float, double), 논리(boolean)데이터타입
 			
 		2) 참조타입(reference type)
 		
 			객체가 저장되어 있는 메로리의 주소를 참조하는 타입으로 Array(배열), Enum(열거),
 		...
 		3) 기본 vs 참조
 			...
 			변수에 저장되는 것이 아니라 문자열을 String객체가 생성할때 그 객체가 Heap메모리영역에
 			생성되고 String변수는 String객체가 저정되어 있는 힙영역의 메모리주소를 참조한다.
 			
 			Java는 문자리터럴이 동일하다면 String리터럴을 유하도록 설계되어 있다. 일반적으로 변수에
 			문자를 저장할 경우에는 문자열리터럴을 사용하지만 객체생성연산자ㅏ인 new를 사용해서 직접
 			string객체를 생성할 수 있다.
 	...
 	2. 메모리 사용영역
 		...
 		2) Heap(힙)영역
 			...
 			스택영역의 변수나 다른 객페의 필드에서 참조한다. 참조하는 변수나 필드가 없다면 의미가
 		...
 		3) Method(메서드)영역
 			...
 			상수, 필드, 메서드, 생성자등을 구분해서 메서드영역에 저장해 둔다. 이영역은 모든
 			쓰레드(프로그램)가 공유되는 영역이다.
...
public class ReferenceTypeMain {
	...
	public static void main(String[] args) {
		...
		String str2 = "홍길동";							// 리터럴 - C.P영역(str2의 러터럴은 str1의 리터럴과 동일값)	
		String str3 = new String("홍길동");				// 객체 - 힙영역
		String apt32 = new String("삼성레미안32평");	
		String apt40 = new String("푸르지오40평");
