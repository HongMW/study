...
/*
 	...
 	Java에서는 인터페이스는 개체의 사용방법을 정의한 설계도이다. interface도 참조타입이다.
 	인터페이스는 객체의 교호나성(형변환)을 높여 주기 때문에 다형성을 구현하는 매우 중요한
...
public interface RemoteControl {
	...
	int min_volume = 0; // 생략시에는 public static final이 자동추가
	...
	// 기 때문에 생략하더라고 컴파일시에 자동으로 public abstract가 추가 된다.
	void powerOn();
	void powerOff();
	...
	// 를 확장해서 새로운 기능을 추가학기 위해서이다. default메서드는 실행블럭을
	// 가지고 있는 메서드로서 default키워드를 반드시 붙여야 하며 기본적으로 public
	// 접근제한자를 가지고 있고 생략하더라도 컴파일러가 자동으로 추가한다
	...
	// 4. static메서드(인터페이스멤버)
	// 정덕메서드도 Java8버전부터 사용할 수 있는데 default메서드와는 달리 객체가
	// 생성되지 않아도 사용할 수 있는 메서드이다. 즉, 객체멤버가 아니라 인터페이스
	// 멤버로서 "인터페이스명.메서드명"의 형태로 호출할 수가 있다.
