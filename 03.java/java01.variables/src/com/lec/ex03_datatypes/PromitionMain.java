...
/*
 	...
 	모든 리터럴과 변수는 테이터타입이 있다. 프로그램을 작성하다 보면 서로 다른 데이터타입의 값을
 	연산하는 경우가 자주 발생하는데 모든 연산은 기본적으로 동일 타입의 데이터만 연산할 수 있다.
 	만약, 서로 다른 타입의 연산을 수행하는 경우에는 연산을 수행하기 전에 형변환을 통해 동일 타입의
 	데이터타입을 변환해야 한다.
 	
 	형변환이란? 변수 또는 리터럴의 데이터타입을 다른 데이터타입으로 변화하는 것을 말한다. 큰 크기의
 	데이터타입에서 작은 크기의 데이터타입으로 변환하는 것을 "강제형변환(casting, 명시적)이라고 하고
 	작은 크기에서 큰 크기의 데이터타입으로 변환하는 것을 "자동형변환(Promotion. 묵시적)"이라고 한다.
