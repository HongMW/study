...
/*
	...
	1.변수란?
		...
		변수라는 용어는 프로그램에서 수시로 ㄱ밧이 변동되면서 저장된 값이 변하기 때문에 변수라고
		명명 되었다.
		
	2. 변수의 명명규칙
	
		1) 첫 번째 문자는 문자,$,언더바(_)로 시작해야 하고 숫자로 시작할 수 없다.(필수)
		2) 변수명은 대문자와 소문자 구분이된다(필수) firstName과 FirstName 다른 변수이다.
		3) 첫 번째 문자는 소문자로 시작하고 단어 구분마다 대문자로 선언(camel case방식, 관례)
		4) 변수명의 길이는 제한이 없다.
		5) 한글변수명도 가능하다. 하지만 사용하지 않는다.(관례)
		6) 자예약어(if, for...)는 사용할 수 없다(필수)
...
public class VariableMain1 {
	...
	public static void main(String[] args) {
		...
		int firstNumber = 10;
		//int firstNumber = 20; // 에러 : Java에서는 한번 선언된 변수는 재선언 할 수가 없다.
