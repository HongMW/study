...
public class LiteralMain {
	...
	public static void main(String[] args) {
		...
		// 16진수 : 숫자 맨 앞에 0x으로 시작되는 리터럴
		int var3 = 0x10;
		System.out.println("16진수 = " + var3);
		
		// 2진수 : 숫자 맨앞에 0b으로 시작되는 리터럴
		int var4 = 0b10;
		...
		char chr1 = 'A'; // 내부적 65
		int int1 = 10;
		System.out.println("chr1 + int1 = " + (chr1 + int1)); // 연산시에는 큰 타입으로 자동형변환이 된다.
		System.out.println("chr1 + int1 = " + (chr1 + (char) int1)); // int타입을 char타입으로 강제 형변환
		System.out.println();
		
		double d1 = 10.0;
		int1 = 65;
		System.out.println("d1 + int1" + (d1 + int1)); // 자동형변환
		
		// 기본적으로 작은 크기의 타입과 큰 크기의 타입 연산결과는 큰 데이터타입으로
		// 자동형변환이 된 후에 연산이 된다.
