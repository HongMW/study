...
/*
 	...
 	프로그램을 개발하다보면 작게는 수십개, 많게는 수백개의 클래스를 작성해야 한다.
 	...
 	저장해서 우사한 성격의 파일들을 그룹화해서 관리하듯이 Java에서는 패키지를 만들어
 	유사한 기능을 하는 클래스들끼리 그룹화해서 저장, 관리하도록 한다.
 	
 	패키지의 물리적은 형태는 파일시스템의 폴더이다. 패키지는 단순히 파일시스템의 폴더
 	기능만 하는 것이 아니라 "클래스이름의 일부분"이다. 즉 클래스이름은 패키지명을 포함한
 	클래스파일명 전체가 클래스 이름이다.
 	
 	클래스 전체이음은 "페키지명.하위패키지명.클래스명" 인데 상위 하위로 구분이 되어 있다명
 	dot(.)로 구분한다.
