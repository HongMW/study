...
public class ArraysAsListMain {
	...
	public static void main(String[] args) {
		...
		// ArrayList는 일반배열처럼 초히화기능 없다.
		// 대신에 Arrays.asList(값, ... 값n)을 이용해서 초기화를 한다.
		// ArrayList를 생성하고 런타임시에 필요에 따라 객체를 추가하는 방법이 일반적이지만
		// 고정된 객체로 구성된 List를 생성할 경우에는 Arrays.asList()메서드를 사용하는 것이
		// 간편할 수 있다.
		// 다만, Array.asList()메서드로 생성된 ArrayList는 크기를 변경할 수 없다.
		...
		// 다만 asList()메서드로 생성된 List는 고정된 객체로 생서오딘다. 즉, 추가, 삭제는
		// 불가능하다.
		...
		// list2.add("이강인"); // (x) UnsupportedOperationExceotion
		// list2.remove(0);	// (x) UnsupportedOperationExceotion
