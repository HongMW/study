...
/*
	...
	2. Connection객체를 생성
		...
		Connection객체에 변결하는 것은 DriverManager에 등록된 각 드라이버를 getConnection()
		메서드를 이용해서 식별한다. gerConnection(db url주소, username, password)의 매개값을
		전달한다. 연결할 때 url식별자 정보를 이용해서 Mapping을 하고 찾지 못햇을 경우에는
		...
		c. MtSQL : getConnection("jdbc:mysql://MySQL주소", "userID", "password");
	
	3. Statement(Statement/PreparedStatement/CallableStatement)
	
		SQL생성 및 실행하면 반환된 경과를 가져오게할 작업영역을 제공한다. Statement 객체는
		Connection객체의 createStatement()메서드를 사용하여 생성한다.
		
		이 단계부터는 JDBC드라이버에 구애를 받지 않는다. 즉, DB상관없이 동일한 명령으로
		사용할 수 있다. 단 SQL표준문법을 준후할 경우 동일한 명령이지만 DB마다 고유한 명령
	...
	4. Query실행
		...
		ResultSet rs = stmt.excuteQuery("select * from emp where depno = 10");
		ResultSet rs = pstnt.excuteQuery("select * from emp where depno = ?");
		ResultSet rs = cstmt.excute();
	...
	5. ResultSet처리
		...
		데이터를 추출하는 방법은 ResultSet에서 한 행(row)씩 이도하면서 getXXX()메서드를
		이용하여 데이터를 추출하는데 rs.getString(1) or rs.getInt("컬럼명")의 방법으로
		추출한다.
