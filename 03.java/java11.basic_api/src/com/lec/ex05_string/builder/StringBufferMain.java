...
/*
	...
	문자열을 저장하는 String클래스는 내부의 문자열을 수정할 수 없다. 예를 들어서 String.relace()
	메서드는 내부의 문자열을 변경하는 것이 아니라 새롭게 대체된 문자열객체를 리턴한다.
	또한 예를 들어 "JAVA" + "PROGRAMMING"을 실행하는 경우 내부문자열을 수정할수 없기 때문에
	"JAVA PROGRAMMING"이라는 새로운 String객체를 생성한 후에 그 객체의 참조번지를 리턴한다. 문자열
	...
	클래스를 사용하는 것이 좋다. 이 두 클래스는 내부버퍼(buffer, 데이터를 임시로 저장하는 메모리)에
	...
	1. append : 문자열 끝네 매개값으로 주어진 문자열 추가
	2. insert : 문자열사이에 매개값으로 주어진 문자열을 삽입
