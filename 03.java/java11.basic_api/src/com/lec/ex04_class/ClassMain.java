...
/*
	...
	1. Class객체얻기(Ckass.forName(String str), Object.getClass(클래스이름)
		...
		호출할 수가 있다. getClass()메서드는 해당 클래스ㅡ이 객체가 생성되어 있을 때
		에만 사용할 수 있지만 객체가 생성되어 있지 않다면 class객체를 얻을 수가 있다.
		
		Class의 생성자는 private으로 선언되어 있기 때문에 new객체 생성연산자로 객체를
		...
		Class.forName()메서드는 클래스를 찾지 못했을 경우에 ClassNotFoundException이
		발생시키기 때문에 예외처리를 해야 한다.
		
	2. Class정보얻기
	
		getDeclareConstructors(), getDeclarefields(), getDeclareMethods()등의 메서드를
		이용해서 해당 클래스의 필드, 생성자, 메서드에 대한 정보를 알 수가 있다.
		이것을 리플렉션(Reflection)
	
	3. 동적으로 객체를 생성(newinstance())
	
		Class객체를 이용하면 new연산자를 사용하지 않아도 동작으로 객체를 생성할 수 있다.
		이 방법은 코드 작성시에 클래스의 이름을 결정할 수 ㅇ벗고 런타임시에 클래스 이름이
